...
const WhatWeBuild: FC<ServiceCapabilitiesProps> = ({ heading, items }) => {
  return (
    <section className="relative overflow-hidden bg-[#009f8b] py-16  font-nunito">
        ...
        <div className="grid gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => (
            ...
            >
              ...
              <span className="absolute -right-4 -top-6 text-[100px] font-black leading-none text-white/5 transition-colors duration-300 group-hover:text-[#009f8b]/10">
                {(index + 1).toString().padStart(2, "0")}
              </span>

              {/* Content */}
              <div className="relative z-10">
                <h3 className="mb-4 text-xl font-bold text-white transition-colors duration-300 group-hover:text-[#009f8b]">
                  {item.title}
                </h3>
              ...
              <div className="relative z-10 mt-8">
                <div className="h-0.5 w-12 bg-white/30 transition-all duration-300 group-hover:w-full group-hover:bg-[#009f8b]" />
              </div>
            </div>
          ))}
        </div>
